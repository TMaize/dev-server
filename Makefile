...
clean:
	...
	mkdir -p dist
win:
	GOOS=windows GOARCH=amd64 go build -o ./dist/dev-server.exe main.go
	cd dist && 7z a -sdel dev-server-windows-x64.zip dev-server.exe
linux:
	GOOS=linux GOARCH=amd64 go build -o ./dist/dev-server main.go
...
mac:
	GOOS=darwin GOARCH=amd64 go build -o ./dist/dev-server main.go
